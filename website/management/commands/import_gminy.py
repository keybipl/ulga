...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "plik_excel", type=str, help="Ścieżka do pliku Excel z danymi gmin"
        )
        parser.add_argument(
            "--aktualizuj",
            action="store_true",
        ...
        for index, row in df_filtrowane.iterrows():
            try:
                ...
                gmina, created = Gmina.objects.update_or_create(
                    kod_teryt=kod_teryt, defaults=dane_gminy
                )
                ...
                if created:
                    ...
                    zaktualizowane += 1
                    self.stdout.write(
                        f"  🔄 Zaktualizowano: {gmina.nazwa} ({kod_teryt})"
                    )
                else:
                    pominiete += 1
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⏭️  Pominięto (już istnieje): {gmina.nazwa} ({kod_teryt})"
                        )
                    )
